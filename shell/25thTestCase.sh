...
echo "Stopping the portworx in $node2..."
ssh_machine $node2 sudo systemctl stop portworx
node2_pwx_=$(ssh_machine $node2 sudo systemctl is-active portworx)
if [ "$node2_pwx" != "active" ]
then
    ...
    do 
      ...
      echo "portworx volume list from $node2"
      ssh_machine $node1 sudo /opt/pwx/bin/pxctl v l 
      echo "portworx volume list from $node1"
